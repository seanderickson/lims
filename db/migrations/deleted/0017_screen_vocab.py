...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        j=k=0
        for i,obj in enumerate(orm.Screen.objects.all()):
            attr = 'species'
            ...
            obj.save()
        logger.info(str(('migrated',i,'screens',j,'species vocabs',k,'assay_type vocabs')))
        
        j=0
        for i,obj in enumerate(orm.DataColumn.objects.all()):
            attr = 'assay_readout_type'
            temp = getattr(obj, attr)
            if temp:
                temp2 = default_converter(temp)
                setattr(obj, attr, temp2)
                logger.debug(str(( 'datacolumn', attr, temp,temp2))) 
                j += 1
            obj.save()
        logger.info(str(('migrated',i,'datacolumns',j,'assay readout type vocabs')))
        
        funding_supports = []
        for obj in orm.FundingSupport.objects.all():
            attr = 'value'
            temp = getattr(obj, attr)
            if temp:
                temp2 = default_converter(temp)
                setattr(obj, attr, temp2)
                logger.debug(str(( 'funding_support', attr, temp,temp2))) 
                funding_supports.append((temp2,temp))
            obj.save()
        
        for i,(key,val) in enumerate(funding_supports):
            vocab = Vocabularies() 
            vocab.scope = 'screen.funding_support'
            vocab.key = key
            vocab.title = val
            vocab.json_field = '{"description": "%s" }' % val
            vocab.ordinal = i
            vocab.save()
        
        # FIXME: not done: set funding support to screen
        # FIXME: review this migration to complete all screen vocabs! 20150722
        
        logger.info(str(('migrated', len(funding_supports), 'screen funding supports')))
