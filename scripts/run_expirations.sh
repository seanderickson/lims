...
# credential_file="${SUPPORTDIR}/production_data/sde_credentials.txt"
# BOOTSTRAP_PORT=53999
# MAIL_RECIPIENT_LIST='admin@email.com'
...
# ADMIN_FROM_EMAIL='screensaver-feedback@yoursite.edu'

# CONTACT_PRINTED_INFO='Director (director@yoursite.edu)'

source ${SCRIPTPATH}/expiration.properties
...
function test_setup {
  ...
  -v $TEST_RUN_SETTINGS >>"$LOGFILE" 2>&1
...
function run_expiration_scripts {
  ...
  echo "run expirations using a local dev server on port $BOOTSTRAP_PORT..." >>"$LOGFILE" 2>&1
  nohup $DJANGO_CMD runserver --settings=lims.settings-server-commandline --nothreading \
  --noreload $BOOTSTRAP_PORT  &
